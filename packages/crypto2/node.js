...
const hash = async (algorithm, data) => {
  ...
  const buffer = crypto.createHash(algorithm).update(data).digest();
  const response = Uint8Array.from(buffer);
  return response;
};
...
const hmac = async (algorithm, key, data) => {
  ...
  assert(data instanceof Uint8Array);
  const buffer = crypto.createHmac(algorithm, key).update(data).digest();
  const response = Uint8Array.from(buffer);
  return response;
};
